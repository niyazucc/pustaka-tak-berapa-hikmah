<div class="w-full max-w-sm rounded-lg  ">
    ...
    <div class="px-1 ">
        <a href="{{ route('bookDetails', ['id' => $book->id]) }}">
            ...
            {{-- @if ($book->discount->isNotEmpty())
                    <h5 class="tracking-tight text-black mb-4">Discount: {{ $book->discount->first()->discount_rate }}
                    </h5>
                @endif --}}
        </a>
        ...
        <div class="grid">
            @if ($book->discount->isNotEmpty())
                <span class="text-xl font-bold text-black mb-4">RM
                    {{ $book->price * (1 - ($book->discount->first()->discount_rate / 100)) }} <span
            ...
            @if ($book->discount->isNotEmpty())
                @livewire('addtocart', [
                    'bookid' => $book->id,
                    'quantity' => 1,
                    'price' => $book->price * (1 - ($book->discount->first()->discount_rate / 100)),
                ])
            @else
                @livewire('addtocart', [
                    'bookid' => $book->id,
